...
class APCHomeAccessory {
    constructor(platform, accessory, config, log, device, apcService) {
        ...
        for (let i = 0; i < this.device.outlets.length; i++) {
            ...
            var service = this.accessory.getService(this.device.outlets[i].name);
            if (service == null) {
                service = this.accessory.addService(this.platform.Service.Outlet, this.device.outlets[i].name, this.device.outlets[i].name);
            }
